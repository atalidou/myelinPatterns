%% This code computes the membrane potential along axons of callosal myelin
%% motifs. The percentage of myelin coverage has been set to 70%.
%
% The axons are constructed using the Gamma distribution. For the numerical
...
perMyel = 70;                               % percentage of myelin coverage (PMC)
            ...
            for j=1:NX+1
                ...
                if (spReady(r) == 1)    % if ready to spike, then check for spike occurrence
                    if (soln(r) >= Vsp) 
                        ...
                        spReady(r) = 1; % still ready for spiking
                    end
...
numMyel = randi([44, 350],1);          % number of myelinated segments along the reference axon (sample a uniformly distributed pseudorandom integer)     
...
while true
    ...
    if (sum(myel) == myelAxon && discretize(mean(myel), [20, 160]) == 1 && min(myel)>=1) % condition
        %disp('myelin')
        break
    end 
end  
...
while true
    kappaNode = randi([1,40],1);
    thetaNode = randi([1,20],1);
