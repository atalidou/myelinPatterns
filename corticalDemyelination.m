...
%% axons of cortical myelin motifs.
%
...
for trial = 1:numberOfAxons
    ...
    [ix_affectedD1, ix_unaffectedD1, node_demyelD1, ix_demyelD1, numMyelD1, numNodeD1, myelD1, nodeD1, interfaceD1, interfaceAllD1] = axonDemyel_D1_cortical(axonLength, numMyel, numNode, myel, node, damage, myelExtra, nodeExtra);
        ...
        for i=1:2:m
            for j=1:NX+1
                ...
                if (spReady_D1(r) == 1)    % if ready to spike, then check for spike occurrence
                    if (soln_D1(r) >= Vsp) 
                        spStart_D1(r) = 1; % started spiking
                        spReady_D1(r) = 0; % not ready for spiking
                    else 
                        spStart_D1(r) = 0; % not started spiking
                        spReady_D1(r) = 1; % ready for spiking
                    end
                else                    % if in repolarizing phase, then check whether voltage is back near rest
                    if (soln_D1(r) <= Vre)
                        spStart_D1(r) = 0; % not starting spiking
                        spReady_D1(r) = 1; % ready for next spike
                    else
                        spStart_D1(r) = 0; % not starting spiking
                        spReady_D1(r) = 0; % not ready for next spike
                    end
                end
            end
        end
    ...
    [ix_affectedD2, ix_unaffectedD2, node_demyelD2, ix_demyelD2, numMyelD2, numNodeD2, myelD2, nodeD2, interfaceD2, interfaceAllD2] = axonDemyel_D2_cortical(axonLength, numMyelD1, numNodeD1, myelD1, nodeD1, damage, myelExtra, nodeExtra);
        ...
        for i=1:2:m
            for j=1:NX+1
                ...
                if (spReady_D2(r) == 1)    % if ready to spike, then check for spike occurrence
                    if (soln_D2(r) >= Vsp) 
                        spStart_D2(r) = 1; % started spiking
                        spReady_D2(r) = 0; % not ready for spiking
                    else 
                        spStart_D2(r) = 0; % not started spiking
                        spReady_D2(r) = 1; % ready for spiking
                    end
                else                    % if in repolarizing phase, then check whether voltage is back near rest
                    if (soln_D2(r) <= Vre)
                        spStart_D2(r) = 0; % not starting spiking
                        spReady_D2(r) = 1; % ready for next spike
                    else
                        spStart_D2(r) = 0; % not starting spiking
                        spReady_D2(r) = 0; % ready for next spike
                    end
                end
            end
        end
    ...
    [ix_affectedD3, ix_unaffectedD3, node_demyelD3, ix_demyelD3, numMyelD3, numNodeD3, myelD3, nodeD3, interfaceD3, interfaceAllD3] = axonDemyel_D3_cortical(axonLength, numMyelD2, numNodeD2, myelD2, nodeD2, damage, myelExtra, nodeExtra);
        ...
        for i=1:2:m
            for j=1:NX+1
                ...
                if (spReady_D3(r) == 1)    % if ready to spike, then check for spike occurrence
                    if (soln_D3(r) >= Vsp) 
                        spStart_D3(r) = 1; % started spiking
                        spReady_D3(r) = 0; % not ready for spiking
                    else 
                        spStart_D3(r) = 0; % not started spiking
                        spReady_D3(r) = 1; % ready for spiking
                    end
                else                    % if in repolarizing phase, then check whether voltage is back near rest
                    if (soln_D3(r) <= Vre)
                        spStart_D3(r) = 0; % not starting spiking
                        spReady_D3(r) = 1; % ready for next spike
                    else
                        spStart_D3(r) = 0; % not starting spiking
                        spReady_D3(r) = 0; % not ready for next spike
                    end
                end
            end
        end
    ...
    [ix_affectedD4, ix_unaffectedD4, node_demyelD4, ix_demyelD4, numMyelD4, numNodeD4, myelD4, nodeD4, interfaceD4, interfaceAllD4] = axonDemyel_D4_cortical(axonLength, numMyelD3, numNodeD3, myelD3, nodeD3, damage, myelExtra, nodeExtra);
        ...
        for i=1:2:m
            for j=1:NX+1
                ...
                if (spReady_D4(r) == 1)    % if ready to spike, then check for spike occurrence
                    if (soln_D4(r) >= Vsp) 
                        spStart_D4(r) = 1; % started spiking
                        spReady_D4(r) = 0; % not ready for spiking
                    else 
                        spStart_D4(r) = 0; % not started spiking
                        spReady_D4(r) = 1; % ready for spiking
                    end
                else                    % if in repolarizing phase, then check whether voltage is back near rest
                    if (soln_D4(r) <= Vre)
                        spStart_D4(r) = 0; % not starting spiking
                        spReady_D4(r) = 1; % ready for next spike
                    else
                        spStart_D4(r) = 0; % not starting spiking
                        spReady_D4(r) = 0; % not ready for next spike
                    end
                end
            end
        end
    ...
    [ix_affectedD5, ix_unaffectedD5, node_demyelD5, ix_demyelD5, numMyelD5, numNodeD5, myelD5, nodeD5, interfaceD5, interfaceAllD5] = axonDemyel_D5_cortical(axonLength, numMyelD4, numNodeD4, myelD4, nodeD4, damage, myelExtra, nodeExtra);
        ...
        for i=1:2:m
            for j=1:NX+1
                ...
                if (spReady_D5(r) == 1)    % if ready to spike, then check for spike occurrence
                    if (soln_D5(r) >= Vsp) 
                        spStart_D5(r) = 1; % started spiking
                        spReady_D5(r) = 0; % not ready for spiking
                    else 
                        spStart_D5(r) = 0; % not started spiking
                        spReady_D5(r) = 1; % ready for spiking
                    end
                else                    % if in repolarizing phase, then check whether voltage is back near rest
                    if (soln_D5(r) <= Vre)
                        spStart_D5(r) = 0; % not starting spiking
                        spReady_D5(r) = 1; % ready for next spike
                    else
                        spStart_D5(r) = 0; % not starting spiking
                        spReady_D5(r) = 0; % not ready for next spike
                    end
                end
            end
        end
    ...
    [ix_affectedD6, ix_unaffectedD6, node_demyelD6, ix_demyelD6, numMyelD6, numNodeD6, myelD6, nodeD6, interfaceD6, interfaceAllD6] = axonDemyel_D6_cortical(axonLength, numMyelD5, numNodeD5, myelD5, nodeD5, damage, myelExtra, nodeExtra);
        ...
        for i=1:2:m
            for j=1:NX+1
                ...
                if (spReady_D6(r) == 1)    % if ready to spike, then check for spike occurrence
                    if (soln_D6(r) >= Vsp) 
                        spStart_D6(r) = 1; % started spiking
                        spReady_D6(r) = 0; % not ready for spiking
                    else 
                        spStart_D6(r) = 0; % not started spiking
                        spReady_D6(r) = 1; % ready for spiking
                    end
                else                    % if in repolarizing phase, then check whether voltage is back near rest
                    if (soln_D6(r) <= Vre)
                        spStart_D6(r) = 0; % not starting spiking
                        spReady_D6(r) = 1; % ready for next spike
                    else
                        spStart_D6(r) = 0; % not starting spiking
                        spReady_D6(r) = 0; % not ready for next spike
                    end
                end
            end
        end
...
function [ix_affectedD1, ix_unaffectedD1, node_demyelD1, ix_demyelD1, numMyelD1, numNodeD1, myelD1, nodeD1, interfaceD1, interfaceAllD1] = axonDemyel_D1_cortical(axonLength, numMyel, numNode, myel, node, damage, myelExtra, nodeExtra)
...
function [ix_affectedD2, ix_unaffectedD2, node_demyelD2, ix_demyelD2, numMyelD2, numNodeD2, myelD2, nodeD2, interfaceD2, interfaceAllD2] = axonDemyel_D2_cortical(axonLength, numMyelD1, numNodeD1, myelD1, nodeD1, damage, myelExtra, nodeExtra)
...
function [ix_affectedD3, ix_unaffectedD3, node_demyelD3, ix_demyelD3, numMyelD3, numNodeD3, myelD3, nodeD3, interfaceD3, interfaceAllD3] = axonDemyel_D3_cortical(axonLength, numMyelD2, numNodeD2, myelD2, nodeD2, damage, myelExtra, nodeExtra)
...
function [ix_affectedD4, ix_unaffectedD4, node_demyelD4, ix_demyelD4, numMyelD4, numNodeD4, myelD4, nodeD4, interfaceD4, interfaceAllD4] = axonDemyel_D4_cortical(axonLength, numMyelD3, numNodeD3, myelD3, nodeD3, damage, myelExtra, nodeExtra)
...
function [ix_affectedD5, ix_unaffectedD5, node_demyelD5, ix_demyelD5, numMyelD5, numNodeD5, myelD5, nodeD5, interfaceD5, interfaceAllD5] = axonDemyel_D5_cortical(axonLength, numMyelD4, numNodeD4, myelD4, nodeD4, damage, myelExtra, nodeExtra)
...
function [ix_affectedD6, ix_unaffectedD6, node_demyelD6, ix_demyelD6, numMyelD6, numNodeD6, myelD6, nodeD6, interfaceD6, interfaceAllD6] = axonDemyel_D6_cortical(axonLength, numMyelD5, numNodeD5, myelD5, nodeD5, damage, myelExtra, nodeExtra)
